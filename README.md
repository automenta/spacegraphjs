...
3.  **Include in HTML:**
    ...
    When using the ES Module build (e.g., `dist/spacegraph.esm.min.js`), `three.js` and `gsap` are **not** bundled and **must be provided externally**. The example below demonstrates how to do this using an `importmap`.

    The example below includes an `importmap` to provide `three` and `gsap` from a CDN. This (or a similar mechanism like local installation and import via bundler) is **required** for `spacegraph-zui` to function correctly.
