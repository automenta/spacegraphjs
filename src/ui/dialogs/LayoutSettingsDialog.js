...
export class LayoutSettingsDialog {
  ...
  _createDialogElement() {
    ...
    this.layoutSettingsDialogElement.addEventListener("pointerdown", (e) =>
      e.stopPropagation(),
    );
    ...
    $(
      "#apply-layout-button",
      this.layoutSettingsDialogElement,
    )?.addEventListener("click", this._onApplyLayout);
    $(
      "#close-layout-dialog",
      this.layoutSettingsDialogElement,
  ...
  _updateContent() {
    ...
    controlsHTML += `
        ...
        `;
    $(".layout-controls", this.layoutSettingsDialogElement).innerHTML =
      controlsHTML;
  ...
  _getLayoutDescription(layoutName) {
    const descriptions = {
      force:
        "Simulates physical forces to arrange nodes. Good for organic networks and discovering structure.",
      circular:
        "Arranges nodes in a circle. Useful for highlighting a central node or showing cyclical relationships.",
      grid: "Aligns nodes in a regular grid pattern. Good for structured, uniform data.",
      hierarchical:
        "Organizes nodes in a tree-like structure. Ideal for parent-child relationships.",
      radial:
        "Positions nodes in concentric circles radiating from a central point. (Basic implementation)",
      treemap:
        "Displays hierarchical data as nested rectangles. (Basic implementation)",
      spherical:
        "Arranges nodes on the surface of a sphere. Useful for 3D overviews.",
      ...
      constraint:
        "Positions nodes based on defined constraints (e.g., distance, alignment). Offers fine-grained control.",
      adaptive:
        "Automatically selects and applies an appropriate layout based on graph characteristics.",
      nested:
        "Handles layouts within container nodes. The specific layout used inside a container can vary.",
    };
    return (
      descriptions[layoutName.toLowerCase()] ||
  ...
  dispose() {
    if (this.layoutSettingsDialogElement) {
      $(
        "#apply-layout-button",
        this.layoutSettingsDialogElement,
      )?.removeEventListener("click", this._onApplyLayout);
      $(
        "#close-layout-dialog",
        this.layoutSettingsDialogElement,
      ...
      $(
        "#layout-select",
        this.layoutSettingsDialogElement,
      )?.removeEventListener("change", this._onLayoutSelectionChange);
