...
export class AdvancedHudManager {
  ...
  _createHudElements() {
    ...
    const cameraGroup = this._createMenuGroup(
      "Camera Mode:",
      this.hudModeIndicator,
    );
    ...
    this.hudKeyboardShortcutsButton.innerHTML =
      '⌨️ <span class="label">Shortcuts</span>';
    this.hudKeyboardShortcutsButton.title = "View Keyboard Shortcuts";
    const shortcutsGroup = this._createMenuGroup(
      null,
    ...
    this.hudLayoutSettingsButton.innerHTML =
      '📐 <span class="label">Layout</span>';
    this.hudLayoutSettingsButton.title = "Layout Settings";
    const layoutGroup = this._createMenuGroup(
      null,
      this.hudLayoutSettingsButton,
    );
  ...
  _setupMenuBar() {
    ...
    this.hudLayer.insertBefore(
      this.menuBar.container,
      this.hudLayer.firstChild,
    ); // Prepend if no top region
  }
  ...
  _bindEvents() {
    ...
    this.hudModeIndicator.addEventListener(
      "change",
      this._onModeIndicatorChange,
    );
    this.hudKeyboardShortcutsButton.addEventListener(
      "click",
      this._onKeyboardShortcutsButtonClick,
    );
    this.hudLayoutSettingsButton.addEventListener(
      "click",
      this._onLayoutSettingsButtonClick,
    );
  }
  ...
  _togglePopupMenu = (event) => {
    ...
    if (this.isPopupMenuVisible) {
      ...
      document.addEventListener("click", this._handleClickOutsideMenu, {
        capture: true,
        once: true,
      });
  ...
  _handleClickOutsideMenu = (event) => {
    if (
      !this.hudPopupMenu.contains(event.target) &&
      event.target !== this.hudMainMenuButton
    ) {
      if (this.isPopupMenuVisible) {
  ...
  _getCameraModeDescriptions() {
    ...
    return {
      ...
      firstPerson:
        "First Person (Experimental): Simulates a first-person view. Uses Free Look controls.",
  ...
  _updateCameraModeDescription(modeKey) {
    ...
    this.hudCameraModeDescription.textContent =
      descriptions[modeKey] || "Select a mode to see its description.";
  ...
  updateHudCameraMode(mode) {
    if (this.hudModeIndicator) {
      ...
      if (availableModes) {
        const existingOptions = Array.from(this.hudModeIndicator.options).map(
          (opt) => opt.value,
        );
        const newModeKeys = Object.keys(availableModes);

        if (
          existingOptions.length !== newModeKeys.length ||
          !newModeKeys.every((key) => existingOptions.includes(key))
        ) {
  ...
  _startPerformanceMonitoring() {
    try {
      ...
      console.error(
        "AdvancedHudManager: Failed to start performance monitoring.",
        error,
      );
    ...
    try {
      ...
      if (deltaTime > 0) {
        ...
        this.performanceMetrics.frameTime =
          this.performanceMetrics.frameTime || 0;
      }
      ...
      this.performanceMetrics.nodeCount =
        nodePlugin && typeof nodePlugin.getNodes === "function"
          ? nodePlugin.getNodes().size
      ...
      this.performanceMetrics.edgeCount =
        edgePlugin && typeof edgePlugin.getEdges === "function"
          ? edgePlugin.getEdges().size
          : this.performanceMetrics.edgeCount || 0;
    } catch (error) {
      console.error(
        "AdvancedHudManager: Error updating performance metrics.",
        error,
      );
  ...
  _createAdvancedHudElements() {
    ...
    try {
      ...
      console.error(
        "AdvancedHudManager: Error during initial call to updatePerformanceMetrics",
        error,
      );
    ...
    if (!this.notificationContainer.parentNode) {
      // If base didn't add it or we want to move it
  ...
  _updateSelectionStatus() {
    ...
    const selectedNodes = this._uiPluginCallbacks.getSelectedNodes
      ? this._uiPluginCallbacks.getSelectedNodes()
      : new Set();
    const selectedEdges = this._uiPluginCallbacks.getSelectedEdges
      ? this._uiPluginCallbacks.getSelectedEdges()
      : new Set();
  ...
  _updateLayoutStatus(eventData) {
    ...
    if (eventData && eventData.type) {
      // Check if eventData and type exist
  ...
  _subscribeToAdvancedEvents() {
    ...
    this.space.on("camera:modeChanged", (eventData) =>
      this._updateCameraStatus(),
    ); // mode might be in eventData.newMode

    // Layout events (example event names, verify with SpaceGraph actual events)
    this.space.on("layout:started", (eventData) =>
      this._updateLayoutStatus(eventData),
    );
    this.space.on("layout:stopped", (eventData) =>
      this._updateLayoutStatus(eventData),
    );
    this.space.on("layout:adapted", (eventData) =>
      this._updateLayoutStatus(eventData),
    ); // If layout emits adaptation events
  ...
  _populateDefaultMenus() {
    ...
    fileMenu.addMenuItem("takeScreenshot", "Take Screenshot", () =>
    ...
    const appearanceSection = viewMenu.addSection(
      "appearance",
      "Appearance Options",
      true,
    );
    appearanceSection.addMenuItem(
      "toggleGrid",
      "Grid",
      (checked) => this._toggleGrid(),
      {
        type: "checkbox",
        checked: false,
        updateHandler: (item) => {
  ...
  update() {
    ...
    try {
      if (this.fpsStatusElement && this.performanceMetrics) {
        ...
        const displayFps =
          typeof fpsValue === "number" ? fpsValue.toFixed(0) : fpsValue;
  ...
  _setupPerformancePanel() {
    /* Now managed by _createAdvancedHudElements or as a pinned window */
  }
  ...
  showProgress(id, message, percentage) {
    /* Similar adaptation for progress */
    if (!this.settings.showProgressIndicators || !this.progressContainer)
      return;
  ...
  _toggleFullscreen() {
    ...
    if (!document.fullscreenElement) {
      this.hudLayer.requestFullscreen?.() ||
  ...
  pinSection(section) {
    if (this.pinnedWindows.has(section.id)) {
      ...
      existingWindow.container.style.zIndex =
        AdvancedHudManager.getNextZIndex();
      return;
    }
  ...
  applyHudSettings(settings) {
    ...
    if (this.menuBar) {
      this.menuBar.container.style.display = this.settings.showMenuBar
        ? "flex"
        : "none";
    } else if (this.settings.showMenuBar && !this.menuBar) {
      // This should ideally be caught by the block above, but as a fallback:
      console.warn(
        "AdvancedHudManager: settings.showMenuBar is true, but this.menuBar is still not initialized after attempting setup.",
      );
    }

    if (this.performancePanel) {
      this.performancePanel.style.display = this.settings.showPerformanceMetrics
        ? "block"
        : "none";
    }
    if (this.minimapPanel) {
      this.minimapPanel.style.display = this.settings.showMinimap
        ? "block"
        : "none";
    }
    if (this.statusBar) {
      this.statusBar.style.display = this.settings.showStatusBar
        ? "flex"
        : "none";
    }
    ...
    if (this.crosshairElement) {
      // Assuming a `this.crosshairElement` might exist or be created elsewhere
      this.crosshairElement.style.display = this.settings.showCrosshair
        ? "block"
        : "none";
    }
    ...
    if (this.menuBar) {
      if (settings.showPerformanceMetrics !== undefined) {
        this.menuBar.updateMenuItemState(
          "togglePerformance",
          this.settings.showPerformanceMetrics,
        );
      }
  ...
  dispose() {
    ...
    if (this.performancePanel?.parentNode)
      this.performancePanel.parentNode.removeChild(this.performancePanel);
    if (this.minimapPanel?.parentNode)
      this.minimapPanel.parentNode.removeChild(this.minimapPanel);
