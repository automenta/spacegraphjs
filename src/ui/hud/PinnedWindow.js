...
export class PinnedWindow {
  constructor(hudManager, section) {
    ...
    this.container.style.backgroundColor =
      "var(--graph-background-color-secondary, #282c34)";
    this.container.style.border =
      "1px solid var(--graph-accent-color, #61dafb)";
    ...
    this.header.style.backgroundColor =
      "var(--graph-accent-color-muted, #4a5260)";
  ...
  _makeDraggable() {
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      newX = Math.max(
        0,
        Math.min(newX, parentRect.width - this.container.offsetWidth),
      );
      newY = Math.max(
        0,
        Math.min(newY, parentRect.height - this.container.offsetHeight),
      );
