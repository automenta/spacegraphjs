...
export class ContextMenu {
  ...
  _onContextMenuClick = (e) => {
    ...
    const { action, nodeId, edgeId, positionX, positionY, positionZ } =
      li.dataset;
  ...
  _getContextMenuItemsForNode(node) {
    ...
    items.push({
      label: "🔗 Start Link",
      action: "start-linking-node",
      nodeId: node.id,
    });
    items.push({
      label: "🔎 Auto Zoom",
      action: "autozoom-node",
      nodeId: node.id,
    });
  ...
  _getContextMenuItemsForBackground(worldPos) {
    ...
    if (worldPos) {
      ...
      items.push({
        label: "📄 Add HTML Node",
        action: "create-html-node",
        ...posData,
      });
      items.push({
        label: "📝 Add Note",
        action: "create-note-node",
        ...posData,
      });
      items.push({
        label: "📦 Add Box",
        action: "create-shape-node-box",
        ...posData,
      });
      items.push({
        label: "🌐 Add Sphere",
        action: "create-shape-node-sphere",
        ...posData,
      });
    }
    items.push({ type: "separator" });
