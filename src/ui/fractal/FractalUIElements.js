...
export function setFractalElementActive(fractalMesh, isActive, originalColor, isGrabbed = false) {
    ...
    if (fractalMesh.userData.originalScaleForTransform === undefined) {
        fractalMesh.userData.originalScaleForTransform = fractalMesh.scale.clone();
    }
    ...
    const baseScale = fractalMesh.userData.originalScaleForTransform;

    // Check current zoom level. If zoomed, zoom's emissive takes precedence for emissive calculations.
    ...
    if (isActive) {
        ...
        // Default emissive intensity for hover, will be overridden if grabbed
        let emissiveIntensity = 0.35; // Increased for hover

        if (isGrabbed) {
            activeColorValue = baseColor.clone().lerp(new THREE.Color(0xffffff), 0.6); // Brighter lerp for grab
            activeOpacityValue = Math.min(1, baseOpacity * 1.4); // More opaque for grab
            emissiveIntensity = 0.65; // Stronger emissive for grab

            // Apply subtle scale increase for grab
            if (baseScale) {
                fractalMesh.scale.copy(baseScale).multiplyScalar(1.1);
            }

            if (fractalMesh.material.emissive && !isZoomed) {
            ...
            // activeColorValue = baseColor.clone().multiplyScalar(1.5); // Old method
            activeColorValue = baseColor.clone().lerp(new THREE.Color(0xffffff), 0.25); // Brighter, more luminous
            ...
            activeOpacityValue = Math.min(1, baseOpacity * 1.2); // Slightly more opaque for hover

            if (fractalMesh.material.emissive && !isZoomed) {
                fractalMesh.material.emissive.copy(activeColorValue).multiplyScalar(emissiveIntensity);
            }
            // Reset scale if it was changed by a previous grab, and now it's just hover
            if (baseScale && !fractalMesh.scale.equals(baseScale)) {
                 // Check against original scale, not semantic zoom scale
                 // This needs to be careful if semantic zoom also affects scale directly.
                 // For now, assuming semantic zoom primarily affects child components or non-uniform scale,
                 // and this 'baseScale' is for the hover/grab effect on the object itself.
                 // If applySemanticZoomToAxis also modifies fractalMesh.scale, this logic will conflict.
                 // Current applySemanticZoomToAxis modifies scale of children (cylinder, cone) based on their own originalScale.
                 // So, this should be fine for the parent fractalMesh (e.g. the whole axis group if it were one mesh, or AGH).
                 // For individual axis parts, their scale is managed by applySemanticZoomToAxis.
                 // This scale effect is more suited for the AGH or future whole-tool fractals.
                 // For axis parts, this grab scale might be too much if semantic zoom also scales them.
                 // Let's assume for now this function is called on elements whose base scale isn't managed by semantic zoom directly.
                 // Or, ensure originalScaleForTransform is truly the pre-semantic-zoom scale.
                 // The current plan is to apply this to manipulator parts.
                 // Let's only apply grab scale to AGH for now to avoid conflict.
                if (fractalMesh.name === 'AdaptiveGeometricHub' && baseScale) {
                    // Only apply grab scale reset if not grabbed and it's AGH
                   // fractalMesh.scale.copy(baseScale); // This was causing AGH to shrink.
                   // The baseScale is the initial scale. updateFractalUIScale continuously updates it.
                   // So, scaling for grab should be relative to its current, camera-adjusted scale.
                   // This needs a rethink if we want grab to scale.
                   // For now, removing the grab scale to avoid complexity with updateFractalUIScale and semanticZoom.
                   // Re-evaluate grab scale later if deemed necessary.
                }
            }
        }
        ...
        // Reset scale if it was changed by a previous grab
        // Similar concerns as above regarding interaction with other scaling mechanisms.
        // Removing grab scale for now.
        // if (fractalMesh.name === 'AdaptiveGeometricHub' && baseScale && !fractalMesh.scale.equals(baseScale)) {
        //    fractalMesh.scale.copy(baseScale);
        // }
    ...
    // If it's NOT zoomed, and it became inactive (isActive=false), ensure its emissive is reset to baseEmissive.
    // If it IS zoomed, and it became inactive, its emissive should remain the zoom emissive (handled by applySemanticZoomToAxis).
    // The logic `if (fractalMesh.material.emissive && !isZoomed)` handles the non-zoomed case correctly.
