...
export class UIManager {
  ...
  constructor(space, contextMenuEl, confirmDialogEl, uiPluginCallbacks) {
    ...
    this.hudManager = new AdvancedHudManager(
      this.space,
      this.container,
      this._uiPluginCallbacks,
    );
  ...
  _onSelectionChanged = (payload) => {
    const selectedEdges =
      payload.selected.size > 0 && payload.type === "edge"
        ? payload.selected
        : new Set();
    if (selectedEdges.size === 1) {
      const edge = selectedEdges.values().next().value;
      if (
        !this.edgeMenu.edgeMenuObject ||
        this.edgeMenu.edgeMenuObject.element.dataset.edgeId !== edge.id
      ) {
  ...
  _transitionToState(newState, data = {}) {
    ...
    console.log(
      `UIManager: Exiting state: ${this.currentState}, transitioning to ${newState}`,
    );
    ...
    switch (newState) {
      case InteractionState.DRAGGING_NODE:
        ...
        this.dragOffset = worldPos
          ? worldPos.sub(this.draggedNode.position)
          : new THREE.Vector3();
      ...
      case InteractionState.RESIZING_NODE:
        ...
        this.resizeStartPointerPos = {
          x: this.pointerState.clientX,
          y: this.pointerState.clientY,
        };
        ...
        if (node && cam && node.cssObject) {
          ...
          const worldOrigin = localOrigin
            .clone()
            .applyMatrix4(node.cssObject.matrixWorld);
          const worldOffsetX = localOffsetX
            .clone()
            .applyMatrix4(node.cssObject.matrixWorld);
          const worldOffsetY = localOffsetY
            .clone()
            .applyMatrix4(node.cssObject.matrixWorld);
          ...
          this.resizeNodeScreenScaleX = Math.abs(
            screenOffsetXPx.x - screenOriginPx.x,
          );
          this.resizeNodeScreenScaleY = Math.abs(
            screenOffsetYPx.y - screenOriginPx.y,
          );

          if (this.resizeNodeScreenScaleX < 0.001)
            this.resizeNodeScreenScaleX = 0.001;
          if (this.resizeNodeScreenScaleY < 0.001)
            this.resizeNodeScreenScaleY = 0.001;
        } else {
    ...
    this.space.emit("interaction:stateChanged", {
      newState,
  ...
  _onPointerDown = (e) => {
    ...
    if (this.pointerState.button === 0) {
      if (targetInfo.nodeControls) {
        e.preventDefault();
        e.stopPropagation();
        this._handleNodeControlButtonClick(
      ...
      if (targetInfo.node) {
        ...
        this._transitionToState(InteractionState.DRAGGING_NODE, {
          node: targetInfo.node,
        });
        this._uiPluginCallbacks.setSelectedNode(targetInfo.node, e.shiftKey);
        this.contextMenu.hide();
      ...
      if (targetInfo.intersectedEdge) {
        e.preventDefault();
        this._uiPluginCallbacks.setSelectedEdge(
          targetInfo.intersectedEdge,
          e.shiftKey,
        );
        this.contextMenu.hide();
        return;
    ...
    switch (this.currentState) {
      ...
      case InteractionState.DRAGGING_NODE:
        ...
        if (this.draggedNode) {
          ...
          if (worldPos) {
            const primaryNodeNewCalculatedPos = worldPos
              .clone()
              .sub(this.dragOffset);
            primaryNodeNewCalculatedPos.z = targetZ;

            const dragDelta = primaryNodeNewCalculatedPos
              .clone()
              .sub(this.draggedNode.position);
            const selectedNodes = this._uiPluginCallbacks.getSelectedNodes();

            if (
              selectedNodes?.size > 0 &&
              selectedNodes.has(this.draggedNode)
            ) {
      ...
      case InteractionState.RESIZING_NODE:
        e.preventDefault();
        if (this.resizedNode) {
          const totalDx_screen =
            this.pointerState.clientX - this.resizeStartPointerPos.x;
          const totalDy_screen =
            this.pointerState.clientY - this.resizeStartPointerPos.y;

          const deltaWidth_local =
            totalDx_screen / (this.resizeNodeScreenScaleX || 1);
          const deltaHeight_local =
            totalDy_screen / (this.resizeNodeScreenScaleY || 1);
      ...
      case InteractionState.LINKING_NODE:
        ...
        const targetElement =
          targetInfo.node?.htmlElement ?? targetInfo.node?.labelObject?.element;
        if (
          targetInfo.node &&
  ...
  _onPointerUp = (e) => {
    ...
    if (
      currentInteractionState === InteractionState.LINKING_NODE &&
      e.button === 0
    ) {
      this._uiPluginCallbacks.completeLinking(
        this.pointerState.clientX,
        this.pointerState.clientY,
      );
    }
  ...
  _handleNodeControlButtonClick(buttonEl, node) {
    ...
    const actionClass = [...buttonEl.classList].find(
      (cls) => cls.startsWith("node-") && !cls.includes("button"),
    );
  ...
  _onDocumentClick = (e) => {
    ...
    if (
      this.hudManager.keyboardShortcutsDialog.keyboardShortcutsDialogElement?.contains(
        e.target,
      )
    )
      return;
    if (
      this.hudManager.layoutSettingsDialog.layoutSettingsDialogElement?.contains(
        e.target,
      )
    )
      return;
    ...
    if (this.edgeMenu.edgeMenuObject) {
      ...
      const clickedSelectedEdge =
        targetInfo.intersectedEdge &&
        selectedEdges?.has(targetInfo.intersectedEdge);
  ...
  _onKeyDown = (e) => {
    ...
    const primarySelectedNode =
      selectedNodes.size > 0 ? selectedNodes.values().next().value : null;
    const primarySelectedEdge =
      selectedEdges.size > 0 ? selectedEdges.values().next().value : null;
    ...
    switch (e.key) {
      ...
      case "Escape":
        if (this._uiPluginCallbacks.getIsLinking()) {
          ...
          this.contextMenu.contextMenuElement.style.display === "block"
        ) {
          this.contextMenu.hide();
          handled = true;
        } else if (
          this.confirmDialog.confirmDialogElement.style.display === "block"
        ) {
      ...
      case "Enter":
        if (
          ...
          primarySelectedNode.htmlElement
  ...
  _getTargetInfo(event) {
    ...
    const interactiveEl = element?.closest(
      "button, input, textarea, select, a, .clickable",
    );

    let graphNode = nodeElement
      ? this.space.plugins
          .getPlugin("NodePlugin")
          ?.getNodeById(nodeElement.dataset.nodeId)
      : null;
    ...
    if (needsRaycast) {
      const intersectedObjectResult = this.space.intersectedObjects(
        event.clientX,
        event.clientY,
      );
  ...
  _createTempLinkLine(sourceNode) {
    ...
    this.space.plugins
      .getPlugin("RenderingPlugin")
      ?.getWebGLScene()
      ?.add(this.tempLinkLine);
  }
  ...
  _updateTempLinkLine(screenX, screenY) {
    ...
    const targetPos = this.space.screenToWorld(
      screenX,
      screenY,
  ...
  _removeTempLinkLine() {
    if (this.tempLinkLine) {
      ...
      this.space.plugins
        .getPlugin("RenderingPlugin")
        ?.getWebGLScene()
        ?.remove(this.tempLinkLine);
      this.tempLinkLine = null;
    }
  }

  _onLinkingStarted = (data) => {
    this._transitionToState(InteractionState.LINKING_NODE, {
