...
describe("WidgetComposer", () => {
  ...
  beforeEach(() => {
    mockSpace = {
      addNode: vi.fn((nodeConfig) => ({
        // Mock addNode to return a basic node structure
      ...
      emit: vi.fn(),
      addEdge: vi.fn((source, target, options) => ({
        // Mock addEdge for connectWidgets
        id: `edge-${Date.now()}`,
        source,
        target,
        options,
  ...
  it("should create a monitoring dashboard using static method", () => {
    ...
    const dashboardNode = WidgetComposer.createMonitoringDashboard(
      mockSpace,
      position,
      metrics,
    );
  ...
  it("should create an analytics dashboard using static method", () => {
    ...
    const dashboardNode = WidgetComposer.createAnalyticsDashboard(
      mockSpace,
      position,
      analyticsConfig,
    );
    ...
    expect(dashboardNode.data.widgets.length).toBe(
      analyticsConfig.keyMetrics.length + analyticsConfig.charts.length + 1,
    );
  });
  ...
  it("should create a control center using static method", () => {
    ...
    const controlCenterNode = WidgetComposer.createControlCenter(
      mockSpace,
      position,
      systems,
    );
  ...
  it("should create a game HUD using static method", () => {
    ...
    const hudNode = WidgetComposer.createGameHUD(
      mockSpace,
      position,
      gameConfig,
    );
  ...
  it("should connect widgets using static method", () => {
    ...
    expect(mockSpace.addEdge).toHaveBeenCalledWith(
      mockSourceWidget,
      mockTargetWidget,
      expect.anything(),
    );
  ...
  it("should export and import configuration using static methods", () => {
    ...
    expect(mockSpace.addNode).toHaveBeenCalledWith(
