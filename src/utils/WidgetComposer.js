export class WidgetComposer {
  ...
  static createDashboard(space, position, config) {
    ...
    const dashboardConfig = this._createWidget(
      "meta-widget",
  ...
  static createControlCenter(space, position, systems = []) {
    ...
    const controlWidgets = systems.map(
      (system, index) =>
        library.slider(
          `control-${system.name}`,
          ...
          0,
          100,
        ), // Using slider as a generic control panel example
    );
  ...
  static createWorkflowBuilder(space, position, steps = []) {
    ...
    workflowWidgets.push(
      this._createWidget("control-panel", "workflow-controls", {
  ...
  static createAnalyticsDashboard(space, position, analytics = {}) {
    ...
    if (analytics.keyMetrics) {
      analytics.keyMetrics.forEach((metric, index) => {
        widgets.push(
          this._createWidget("progress", `metric-${index}`, {
            label: metric.name,
    ...
    widgets.push(
      this._createWidget("control-panel", "analytics-controls", {
  ...
  static createFormBuilder(space, position, formConfig = {}) {
    ...
    if (formConfig.fields) {
      formConfig.fields.forEach((field, index) => {
        formWidgets.push(
          this._createWidget("control-panel", `field-${index}`, {
    ...
    formWidgets.push(
      this._createWidget("control-panel", "form-actions", {
        title: "Form Actions",
        controls: [
  ...
  static createDataVisualization(space, position, datasets = []) {
    ...
    datasets.forEach((dataset, index) => {
      widgets.push(
        library.chart(`chart-${index}`, dataset.name, dataset.chartType),
      );
    ...
    widgets.push(
      this._createWidget("control-panel", "viz-controls", {
  ...
  static createGameHUD(space, position, gameConfig = {}) {
    ...
    if (gameConfig.playerStats) {
      hudWidgets.push(
        this._createWidget("control-panel", "player-stats", {
          title: "Player Stats",
          controls: Object.keys(gameConfig.playerStats).map(
            (stat) =>
    ...
    hudWidgets.push(
      library.progressBar("health", "Health", gameConfig.health || 100, 100),
    );
    hudWidgets.push(
      library.progressBar("energy", "Energy", gameConfig.energy || 100, 100),
    );
    hudWidgets.push(library.infoPanel("minimap", "Mini Map", "🗺️"));
    hudWidgets.push(
      library.infoPanel(
        "inventory",
        `Items: ${gameConfig.inventoryCount || 0}`,
        "🎒",
      ),
    );

    hudWidgets.push(
      this._createWidget("control-panel", "game-controls", {
        title: "Game Controls",
        controls: [
  ...
  static propagateData(event, targetWidget, connectionType) {
    ...
    targetWidget.getAllWidgets().forEach((widget) => {
      if (widget.data.controls) {
        const matchingControl = widget.data.controls.find(
          (c) => c.id === controlId,
        );
        if (matchingControl) {
    ...
    return {
      ...
      button: (id, label, text) =>
        this._createWidget("control-panel", id, {
          ...
          controls: [
            { id, type: "button", label: label || text, text: text || label },
          ],
        }),

      progressBar: (id, label, value = 0, max = 100) =>
        this._createWidget("progress", id, {
          label,
          progressType: "bar",
          value,
          max,
