...
export class FractalZoomManager {
  ...
  _onZoomRequest(deltaY) {
    ...
    const newZoomLevel = Math.max(
      ...
      Math.min(
        this.maxZoomIn,
        this.currentZoomLevel + zoomDirection * this.zoomStep,
      ),
    );
  ...
  _calculateZoomLevelFromCamera() {
    ...
    if (
      Math.abs(calculatedZoomLevel - this.currentZoomLevel) >
      this.lodUpdateThreshold
    ) {
  ...
  zoomToLevel(targetLevel, duration = this.transitionDuration) {
    targetLevel = Math.max(
      this.maxZoomOut,
      Math.min(this.maxZoomIn, targetLevel),
    );
  ...
  _updateLOD() {
    ...
    this.space.emit("fractal-zoom:lodUpdated", {
      lodConfig,
  ...
  _updateEdgeLOD(edge, lodConfig) {
    ...
    if (edge.line) {
      ...
      const lineWidthMultiplier = this._getLineWidthMultiplier(
        lodConfig.edgeDetailLevel,
      );
      ...
      const opacityMultiplier = this._getOpacityMultiplier(
        lodConfig.edgeDetailLevel,
      );
    ...
    // Update edge labels
    if (edge.labelObject) {
      edge.labelObject.visible =
        lodConfig.labelsVisible && lodConfig.edgeDetailLevel !== "none";
    }
  }
