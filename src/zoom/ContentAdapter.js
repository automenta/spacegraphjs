...
export class ContentAdapter {
  ...
  _applyContent(node, contentConfig, lodConfig, zoomLevel) {
    ...
    const content = this._getOrGenerateContent(
      contentConfig,
      lodConfig,
      zoomLevel,
    );
  ...
  _cleanupCache() {
    ...
    const toRemove = entries.slice(
      0,
      entries.length - this.config.maxCachedLevels,
    );
...
export class DataContentAdapter extends ContentAdapter {
  ...
  _generateSummary(config) {
    const summary = config.summaryFunction
      ? config.summaryFunction(this.data)
      : "Data Summary";
  ...
  _generateChart(config, zoomLevel) {
    ...
    const dataPoints = Array.isArray(this.data)
      ? this.data.length
  ...
  _generateTable(config, zoomLevel) {
    if (!Array.isArray(this.data))
      return "<div>Data not suitable for table</div>";
