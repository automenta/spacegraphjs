...
export class SpaceGraph {
  ...
  _updateEdgeConstraint(edge, constraintType) {
    ...
    switch (constraintType) {
      case "rigid":
        if (!params.distance) {
          params.distance = edge.source.position.distanceTo(
  ...
  _intersectNonInstancedNodes(raycaster, currentClosest) {
    ...
    if (nonInstancedNodeMeshes.length > 0) {
      ...
      if (
        ...
        const node = this._nodePlugin.getNodeById(
          intersectedMesh.userData?.nodeId,
        );
        if (node) {
  ...
  _intersectInstancedEdges(raycaster, currentClosest) {
    ...
    if (
      intersection &&
      (!currentClosest || intersection.distance < currentClosest.distance)
    ) {
  ...
  _intersectNonInstancedEdges(raycaster, currentClosest) {
    ...
    if (nonInstancedEdgeLines.length > 0) {
      ...
      if (
        ...
        const edge = this._edgePlugin.getEdgeById(
          intersectedLine.userData?.edgeId,
        );
        if (edge) {
  ...
  intersectedObjects(screenX, screenY) {
    ...
    closestIntersect = this._intersectNonInstancedNodes(
      raycaster,
      closestIntersect,
    );

    // Check instanced edges
    closestIntersect = this._intersectInstancedEdges(
      raycaster,
      closestIntersect,
    );

    // Check non-instanced edges
    closestIntersect = this._intersectNonInstancedEdges(
      raycaster,
      closestIntersect,
    );
  ...
  _setupCameraMouseControls() {
    ...
    this.container.addEventListener(
      "contextmenu",
      this._boundHandleContextMenuEvent,
    );
    this.container.addEventListener(
      "mousedown",
      this._boundHandleMouseDownEvent,
    );
    this.container.addEventListener(
      "mousemove",
      this._boundHandleMouseMoveEvent,
    );
    this.container.addEventListener(
      "mouseup",
      this._boundHandleMouseUpOrLeaveEvent,
    ...
    this.container.addEventListener("wheel", this._boundHandleWheelEvent, {
      passive: false,
    });
  }
  ...
  _handleMouseDownEvent(event) {
    ...
    if (cameraControls.cameraMode === "drag_orbit") {
      ...
      cameraControls.cameraMode === "orbit" ||
      cameraControls.cameraMode === "top_down"
    ) {
      if (event.button === 0) {
