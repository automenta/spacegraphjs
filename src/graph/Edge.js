...
export class Edge {
    ...
    _setGradientColors() {
        ...
        if (posAttribute?.count >= 2 && posAttribute.array?.length >= expectedMinLength && posAttribute.array.length % 3 === 0) {
    ...
    update() {
        ...
        this.labelObject && super.update(this.space);
    }
    ...
    _orientArrowhead(arrowheadMesh, direction) {
        if (!arrowheadMesh || direction.lengthSq() <= 0.0001) return;
    ...
    setHighlight(highlight) {
        ...
        if (this.data.gradientColors?.length === 2 && mat.vertexColors) thicknessMultiplier = 2.0;
