...
export class BaseEdge {
    ...
    _createLabel() {
        ...
        return createCSS3DLabelObject(this.data.label, this.id, 'edge-label', styleData, 'edge-label');
    }
    ...
    setHighlight(highlight) {
        ...
        mat.opacity = highlight ? BaseEdge.HIGHLIGHT_OPACITY : BaseEdge.DEFAULT_OPACITY;
        ...
        if (!mat.vertexColors) mat.color.set(highlight ? BaseEdge.HIGHLIGHT_COLOR : this.data.color);
        mat.needsUpdate = true;

        const highlightArrowhead = (arrowhead) => {
            if (arrowhead?.material) {
                arrowhead.material.color.set(highlight ? BaseEdge.HIGHLIGHT_COLOR : this.data.arrowheadColor || this.data.color);
                arrowhead.material.opacity = highlight ? BaseEdge.HIGHLIGHT_OPACITY : BaseEdge.DEFAULT_OPACITY;
            }
        };
    ...
    setHoverStyle(hovered, force = false) {
        ...
        mat.opacity = hovered ? Math.min(1.0, baseOpacity + BaseEdge.DEFAULT_HOVER_OPACITY_BOOST) : baseOpacity;
        mat.linewidth = hovered ? baseThickness * BaseEdge.DEFAULT_HOVER_THICKNESS_MULTIPLIER : baseThickness;
        mat.needsUpdate = true;

        const hoverArrowhead = (arrowhead) => {
            if (arrowhead?.material) {
                const arrowBaseOpacity = BaseEdge.DEFAULT_OPACITY;
                arrowhead.material.opacity = hovered ? Math.min(1.0, arrowBaseOpacity + BaseEdge.DEFAULT_HOVER_OPACITY_BOOST) : arrowBaseOpacity;
            }
        };
