...
export class DocumentNode extends BaseNode {
  ...
  _createLabel() {
    const labelText = this.data.icon
      ? `${this.data.icon} ${this.data.label}`
      : this.data.label;
    const styleData = {
  ...
  getBoundingSphereRadius() {
    return Math.sqrt(
      (this.data.size / 2) ** 2 + ((this.data.size * 1.2) / 2) ** 2,
    );
  }

  viewDocument() {
    if (this.data.documentUrl) {
      this.space?.emit("node:document:view", {
        node: this,
        url: this.data.documentUrl,
      });
      console.log(
        `DocumentNode: Request to view document at ${this.data.documentUrl}`,
      );
    } else {
      console.warn(
        `DocumentNode: No documentUrl specified for node ${this.id}`,
      );
    }
  }
