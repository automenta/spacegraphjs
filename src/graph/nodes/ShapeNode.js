...
export class ShapeNode extends BaseNode {
  ...
  _setupLODLevels() {
    if (this.lodData?.length) {
      ...
      this.mesh.addLevel(
        new THREE.Object3D(),
        this.data.lodDistanceHide ?? 1500,
      );
    }
  }
  ...
  _createMeshForLevel(levelConfig) {
    ...
    const effectiveLevelSize = Number.isFinite(levelConfig.size)
      ? levelConfig.size
      : this.size;
  ...
  _loadGltfModelForLevel(levelConfig, targetGroup) {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const lodLevelEntry = this.mesh.levels.find(
          (l) => l.object === targetGroup,
        );
        ...
        level.object?.traverse((child) => {
          if (child.isMesh && child.material) {
            ...
            child.material.emissiveIntensity =
              selected && child.material.emissive?.getHex() !== 0x000000
                ? 1.0
                : 0.0;
          }
        });
