  ...
  createElement: vi.fn((tagName) => {
    const el = {
      ...
      querySelector: vi.fn((selector) => {
        ...
        if (selector === ".meta-widget-header")
          return { querySelector: vi.fn() }; // Further nesting if needed
        if (selector.startsWith("."))
          return { addEventListener: vi.fn(), style: {} }; // Generic for other buttons/selectors
        return null;
      }),
...
describe("MetaWidgetNode", () => {
  ...
  beforeEach(() => {
    mockSpace = {
      emit: vi.fn(),
      plugins: {
        /* ... mock plugins if needed by MetaWidgetNode ... */
      },
    };
    ...
    childWidgetData1 = {
      id: "progress-child",
      type: "progress",
      data: { label: "Progress 1" },
    };
    childWidgetData2 = {
      id: "control-child",
      type: "control-panel",
      data: { title: "Controls 1" },
    };
  });
  ...
  it("should support different layout types (check initial setup)", () => {
    const flexMetaWidget = new MetaWidgetNode(
      "flex-widget",
      { x: 0, y: 0, z: 0 },
      { layout: "flex-row" },
    );
    ...
    const columnMetaWidget = new MetaWidgetNode(
      "col-widget",
      { x: 0, y: 0, z: 0 },
      { layout: "flex-column" },
    );
  ...
  it("should emit events when widgets are added or removed", () => {
    metaWidget.addWidget(childWidgetData1);
    expect(mockSpace.emit).toHaveBeenCalledWith(
      "meta-widget:widget-added",
    ...
    expect(mockSpace.emit).toHaveBeenCalledWith(
      "meta-widget:widget-removed",
      expect.objectContaining({
        metaWidget: metaWidget,
        widgetId: childWidgetData1.id,
      }),
    );
  });
});
