...
export class ProceduralShapeNode extends ShapeNode {
  ...
  _generateOrganic() {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      ...
      // Add noise
      const noiseValue =
        this._noise3D(vertex.x * 0.1, vertex.y * 0.1, vertex.z * 0.1) * noise;

      // Add bulges
      const bulgeNoise =
        Math.sin(vertex.x * bulges) *
        Math.sin(vertex.y * bulges) *
        Math.sin(vertex.z * bulges);
  ...
  _generateTree() {
    ...
    const createBranch = (startPos, direction, length, currentDepth) => {
      ...
      mesh.position.copy(
        startPos.clone().add(direction.clone().multiplyScalar(length / 2)),
      );
      mesh.lookAt(endPos);
