export class HierarchicalLayout {
  ...
  async init(nodes, edges, config = {}) {
    ...
    const roots = nodes.filter(
      (n) => !this.nodeMap.get(n.id).parent && !n.isPinned,
    );
  ...
  _secondPass(nodeWrapper, currentXOffset, modSum) {
    nodeWrapper.x =
      currentXOffset + nodeWrapper.prelim * this.settings.nodeSeparation;

    let childX =
      currentXOffset -
      ((nodeWrapper.children.length - 1) * this.settings.nodeSeparation) / 2;
    if (
      nodeWrapper.children.length === 1 &&
      nodeWrapper.children[0].children.length === 0
    ) {
    ...
    nodeWrapper.children.forEach((childWrapper, i) => {
      this._secondPass(
        childWrapper,
        childX + i * this.settings.nodeSeparation,
        0,
      );
    });
  }
