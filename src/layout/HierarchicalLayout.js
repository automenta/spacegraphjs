...
export class HierarchicalLayout {
    ...
    async init(nodes, edges, config = {}) {
        ...
        nodes.forEach(node => {
            this.nodeMap.set(node.id, {
                ...
                parent: null,
                level: -1,
                x: 0, y: 0, // Relative x,y within the layout, z is often 0 for 2D hierarchy
        ...
        if (roots.length === 0 && nodes.length > 0) { // Fallback if no clear roots (e.g. cycle or all connected)
    ...
    _firstPass(nodeWrapper, level) {
        ...
        if (nodeWrapper.children.length === 0) { // Leaf node
    ...
    _secondPass(nodeWrapper, currentXOffset, modSum) {
        ...
        if (nodeWrapper.children.length === 1 && nodeWrapper.children[0].children.length === 0) { // Single leaf child directly under parent
    ...
    stop() { /* Static layout */ }
    kick() { /* Static layout */ }
    addNode(node) { /* Requires full recalculation */ }
    removeNode(node) { /* Requires full recalculation */ }
    addEdge(edge) { /* Requires full recalculation if it changes hierarchy */ }
    removeEdge(edge) { /* Requires full recalculation if it changes hierarchy */ }
