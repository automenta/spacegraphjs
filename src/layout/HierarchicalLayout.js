...
export class HierarchicalLayout {
    ...
    async init(nodes, edges, config = {}) {
        ...
        nodes.forEach((node) => {
            this.nodeMap.set(node.id, {
                ...
                x: 0,
                y: 0,
                width: node.getBoundingSphereRadius() * 2,
                prelim: 0,
                modifier: 0,
            });
        });
        ...
        if (!roots.length && nodes.length > 0) {
        ...
        this.nodeMap.forEach((wrapper) => {
            if (!wrapper.node.isPinned) {
                wrapper.node.position.set(wrapper.x, wrapper.y, 0);
            }
        });
    }
    ...
    _firstPass(nodeWrapper, level) {
        ...
        if (!nodeWrapper.children.length) {
    ...
    _secondPass(nodeWrapper, currentXOffset, modSum) {
        ...
        if (nodeWrapper.children.length === 1 && !nodeWrapper.children[0].children.length) {
    ...
    _calculateTreeWidth(nodeWrapper) {
        if (!nodeWrapper || !nodeWrapper.children.length) return nodeWrapper ? nodeWrapper.width : this.settings.nodeSeparation;
        let width = 0;
