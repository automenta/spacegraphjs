...
export class LayoutConnector {
  ...
  unregisterLayoutRegion(regionId) {
    ...
    this.connections.forEach((connection, connectionId) => {
      if (
        connection.sourceRegion === regionId ||
        connection.targetRegion === regionId
      ) {
  ...
  addConnection(sourceNodeId, targetNodeId, options = {}) {
    ...
    if (!sourceRegion || !targetRegion) {
      console.warn(
        `LayoutConnector: Could not find regions for nodes ${sourceNodeId} -> ${targetNodeId}`,
      );
      return null;
    }
  ...
  _createObstaclesFromNodes(nodes, bounds) {
    return nodes.map((node) => ({
      ...
      radius:
        (node.getBoundingSphereRadius?.() || 25) + this.settings.routingPadding,
      type: "circle",
    }));
  }
  ...
  _addIntermediateRoutingNodes() {
    ...
    for (let i = 0; i < regions.length; i++) {
      for (let j = i + 1; j < regions.length; j++) {
        ...
        const midpoint = region1.bounds.center
          .clone()
  ...
  _connectRoutingNodes() {
    ...
    routingNodes.forEach((node1, i) => {
      routingNodes.forEach((node2, j) => {
        ...
        const hasObstacle = this._pathHasObstacle(
          node1.position,
          node2.position,
        );

        if (!hasObstacle && distance < 500) {
  ...
  _lineIntersectsCircle(start, end, circleCenter, radius) {
    ...
    const closestPoint = start
      .clone()
      .add(lineVec.multiplyScalar(clampedProjection));
  ...
  _routeCurved(start, end, connection) {
    ...
    const controlPoint1 = start
      .clone()
      .add(end.clone().sub(start).multiplyScalar(0.33));
    const controlPoint2 = start
      .clone()
      .add(end.clone().sub(start).multiplyScalar(0.66));
    ...
    return this._generateBezierPath(
      start,
      controlPoint1,
      controlPoint2,
      end,
  ...
  _routeBundled(start, end, connection) {
    ...
    this.connections.forEach((otherConnection, otherId) => {
      ...
      if (
        (sourceRegion1 === sourceRegion2 && targetRegion1 === targetRegion2) ||
  ...
  _createBundledPath(start, end, bundleCandidates) {
    ...
    bundleCenter.add(
      perpendicular.multiplyScalar(this.settings.bundlingRadius),
    );
  ...
  _findPath(start, end, connection) {
    ...
    const startNodeId = this._findClosestRoutingNode(
      start,
      connection.sourceRegion,
    );
    const endNodeId = this._findClosestRoutingNode(
      end,
    ...
    while (openSet.size > 0) {
      ...
      currentNodeData.connections.forEach((neighbor) => {
        ...
        if (tentativeGScore < gScore.get(neighborId)) {
          ...
          fScore.set(
            neighborId,
            tentativeGScore + neighbor.position.distanceTo(endNode.position),
          );
  ...
  _bezierPoint(p0, p1, p2, p3, t) {
    ...
    return new THREE.Vector3(
      ...
      oneMinusTCubed * p0.y +
        ...
        tCubed * p3.y,
      oneMinusTCubed * p0.z +
        3 * oneMinusTSq * t * p1.z +
        3 * oneMinusT * tSq * p2.z +
        tCubed * p3.z,
    );
  }
  ...
  updateConnection(connectionId, options = {}) {
    ...
    if (connection.visualElement && options.metadata) {
      ...
      if (options.metadata.opacity !== undefined)
        material.opacity = options.metadata.opacity;
    }
  }
  ...
  getConnectionsForRegion(regionId) {
    return Array.from(this.connections.values()).filter(
      (conn) =>
        conn.sourceRegion === regionId || conn.targetRegion === regionId,
    );
  }
