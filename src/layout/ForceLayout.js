...
export class ForceLayout {
    ...
    _getPlainGravityCenter() {
        const gravityCenter = this.settings.gravityCenter;
        return gravityCenter && typeof gravityCenter.x === 'number'
    ...
    init(initialNodes, initialEdges, config = {}) {
        ...
        if (config) this.settings = { ...this.settings, ...config };
        ...
        this.worker.postMessage({
            ...
            payload: {
                nodes: workerNodes,
                edges: workerEdges,
                settings: { ...this.settings, gravityCenter: this._getPlainGravityCenter() },
            },
        });
    }
    ...
    releaseNode(node) {
        if (this.nodesMap.has(node.id) && !node.isPinned) {
            this.worker.postMessage({
                type: 'updateNodeState',
    ...
    setSettings(newSettings) {
        ...
        this.worker.postMessage({
            type: 'updateSettings',
            payload: { settings: { ...this.settings, gravityCenter: this._getPlainGravityCenter() } },
        });
    }
