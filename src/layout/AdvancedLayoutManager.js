...
export class AdvancedLayoutManager extends LayoutManager {
  ...
  _autoSelectMode(name, config) {
    ...
    const hasContainers = nodes.some(
      (node) => node.data?.isContainer || node.data?.childLayout,
    );

    // Check for constraint requirements
    const hasConstraints = edges.some(
      (edge) => edge.data?.constraintType || edge.data?.constraintParams,
    );
  ...
  addPositionConstraint(nodeId, targetPosition, options = {}) {
    this.constraintSystem.addPositionConstraint(
      nodeId,
      targetPosition,
      options,
    );
  ...
  addAngleConstraint(nodeId1, nodeId2, nodeId3, options = {}) {
    this.constraintSystem.addAngleConstraint(
      nodeId1,
      nodeId2,
      nodeId3,
      options,
  ...
  setLayoutMode(mode) {
    ...
    this.settings.enableConstraints =
      mode === this.layoutModes.CONSTRAINT || mode === this.layoutModes.HYBRID;
    this.settings.enableNesting =
      mode === this.layoutModes.NESTED || mode === this.layoutModes.HYBRID;
    this.settings.enableAdaptive =
      mode === this.layoutModes.ADAPTIVE || mode === this.layoutModes.HYBRID;
  }
