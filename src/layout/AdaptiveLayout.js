...
export class AdaptiveLayout {
    ...
    this.adaptationRules = [
      {
        name: "SmallNodeCount",
        condition: (metrics) =>
          metrics.nodeCount <= this.settings.sizeThresholds.small,
      ...
      },
      {
        name: "HighDensity",
        condition: (metrics) =>
          metrics.density > this.settings.densityThresholds.dense,
        targetLayout: "force",
        priority: 2,
      ...
      },
      {
        name: "GridSuitable",
        condition: (metrics) =>
          metrics.connectionDensity < 0.3 && metrics.nodeCount > 16,
        targetLayout: "grid",
        priority: 4,
      ...
      },
      {
        name: "LargeGraph",
        condition: (metrics) =>
          metrics.nodeCount > this.settings.sizeThresholds.large,
        targetLayout: "force",
        priority: 2,
  ...
  _calculateGraphMetrics(nodes, edges) {
    ...
    const connectionDensity =
      boundingVolume > 0 ? edgeCount / boundingVolume : 0;
  ...
  _selectBestLayout(metrics) {
    ...
    if (applicableRules.length > 0) {
      ...
      console.log(
        `AdaptiveLayout: Selected ${selectedRule.targetLayout} - ${selectedRule.description}`,
      );
  ...
  async _checkForAdaptation() {
    ...
    if (bestLayout !== this.currentLayoutName) {
      console.log(
        `AdaptiveLayout: Adapting from ${this.currentLayoutName} to ${bestLayout}`,
      );
  ...
  forceAdaptation(targetLayout, reason = "manual") {
    if (!this.availableLayouts.has(targetLayout)) {
      console.warn(
        `AdaptiveLayout: Target layout ${targetLayout} not available`,
      );
      return;
    }
