...
export class ConstraintLayout {
  ...
  _solveDistanceConstraint(constraint) {
    ...
    const correction = diff
      .normalize()
      .multiplyScalar(error * constraint.strength * 0.5);

    if (!wrapper1.isFixed) {
      wrapper1.force.add(
        correction
          .clone()
          .multiplyScalar(wrapper2.mass / (wrapper1.mass + wrapper2.mass)),
      );
    }
    if (!wrapper2.isFixed) {
      wrapper2.force.sub(
        correction
          .clone()
          .multiplyScalar(wrapper1.mass / (wrapper1.mass + wrapper2.mass)),
      );
    }
  }
      ...
      wrappers.forEach((other) => {
        ...
        if (distance < constraint.internalSeparation && distance > 0) {
          const repulsion = diff
            .normalize()
            .multiplyScalar((constraint.internalSeparation - distance) * 0.1);
    ...
    constraint.nodeIds.forEach((nodeId) => {
      ...
      if (boundary.type === "box") {
        ...
        if (pos.x < min.x + padding)
          correction.x = (min.x + padding - pos.x) * strength;
        if (pos.x > max.x - padding)
          correction.x = (max.x - padding - pos.x) * strength;
        if (pos.y < min.y + padding)
          correction.y = (min.y + padding - pos.y) * strength;
        if (pos.y > max.y - padding)
          correction.y = (max.y - padding - pos.y) * strength;
        if (pos.z < min.z + padding)
          correction.z = (min.z + padding - pos.z) * strength;
        if (pos.z > max.z - padding)
          correction.z = (max.z - padding - pos.z) * strength;
        ...
        if (distance > radius - padding) {
          correction = diff
            .normalize()
            .multiplyScalar((radius - padding - distance) * strength);
        }
      }
  ...
  async _solveConstraints() {
    for (let iteration = 0; iteration < this.settings.iterations; iteration++) {
      ...
      this.nodeMap.forEach((wrapper) => {
        ...
        const force = wrapper.force
          .clone()
          .multiplyScalar(this.settings.dampingFactor);
  ...
  _resolveCollisions() {
    ...
    for (let i = 0; i < wrappers.length; i++) {
      for (let j = i + 1; j < wrappers.length; j++) {
        ...
        const distance = diff.length();
        const minDistance =
          wrapper1.radius + wrapper2.radius + this.settings.collisionPadding;
