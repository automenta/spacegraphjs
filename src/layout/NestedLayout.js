...
export class NestedLayout {
  ...
  _buildHierarchy(nodes, edges) {
    const containers = nodes.filter(
      (node) => node.data?.isContainer || node.data?.childLayout,
    );
  ...
  _hasParentContainer(containerId) {
    for (const [, containerData] of this.containerHierarchy) {
      if (
        containerData.children.some(
          (child) => child.container.id === containerId,
        )
      ) {
        return true;
      }
  ...
  async _applyContainerLayout(containerData, depth) {
    ...
    const layoutNodes = childNodes.map((node) => ({
      ...node,
      position: this._normalizePositionToContainer(
        node.position,
        containerBounds,
      ),
    }));
    ...
    layoutNodes.forEach((node) => {
      const worldPosition = this._denormalizePositionFromContainer(
        node.position,
        containerBounds,
      );
  ...
  _calculateAvailableSpace(containerBounds) {
    ...
    return {
      min: containerBounds.min
        .clone()
        .add(new THREE.Vector3(padding, padding, padding)),
      max: containerBounds.max
        .clone()
        .sub(new THREE.Vector3(padding, padding, padding)),
      center: containerBounds.center.clone(),
      size: containerBounds.size
        .clone()
        .sub(new THREE.Vector3(padding * 2, padding * 2, padding * 2)),
    };
  }
  ...
  removeNodeFromContainer(node, containerId) {
    ...
    containerData.childNodes = containerData.childNodes.filter(
      (n) => n.id !== node.id,
    );
    if (node.data) {
    ...
    for (let i = 0; i < this.settings.iterations; i++) {
      nodes.forEach((node, index) => {
          ...
          if (distance > 0) {
            force.add(
              diff
                .normalize()
                .multiplyScalar(
                  this.settings.repulsion / (distance * distance),
                ),
            );
          }
        });
        ...
        if (distanceFromCenter > maxDistance) {
          const centerForce = node.position
            .clone()
            .normalize()
            .multiplyScalar(-this.settings.attraction * distanceFromCenter);
...
class HierarchicalLayoutNested {
  ...
  async init(nodes, edges, config = {}) {
    ...
    nodes.forEach((node, index) => {
      ...
      const x =
        (positionInLevel - (nodesPerLevel - 1) / 2) *
