...
function _calculateStepInWorker() {
    ...
    if (settings.enableClustering && (settings.clusterStrength || 0) > 0) {
        ...
        nodes.forEach((node) => {
            ...
            if (!clusters.has(clusterId)) clusters.set(clusterId, { nodeIds: [], centerX: 0, centerY: 0, centerZ: 0, count: 0 });
...
self.onmessage = function (event) {
    ...
    switch (type) {
        ...
        case 'addEdge':
            ...
            break;
        case 'removeEdge':
            edges = edges.filter((e) => !(e.sourceId === payload.edge.sourceId && e.targetId === payload.edge.targetId));
            if (isRunning) lastKickTime = Date.now();
            break;
