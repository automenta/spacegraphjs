...
function _calculateStepInWorker() {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (!nodeA.isFixed)
        nodeForces.set(nodeA.id, vecAdd(nodeForces.get(nodeA.id), forceVec));
      if (!nodeB.isFixed)
        nodeForces.set(nodeB.id, vecSub(nodeForces.get(nodeB.id), forceVec));
    }
  }
  ...
  edges.forEach((edge) => {
    ...
    switch (edge.constraintType) {
      ...
      case "weld":
        forceMag =
          ...
          (distance -
            (params.distance ??
              (sourceNode.radius || 10) + (targetNode.radius || 10)));
        break;
      case "elastic":
      default:
        forceMag =
    ...
    if (!sourceNode.isFixed)
      nodeForces.set(
        sourceNode.id,
        vecAdd(nodeForces.get(sourceNode.id), forceVec),
      );
    if (!targetNode.isFixed)
      nodeForces.set(
        targetNode.id,
        vecSub(nodeForces.get(targetNode.id), forceVec),
      );
  });
  ...
  if (settings.enableClustering && settings.clusterStrength > 0) {
    ...
    clusters.forEach((clusterData) => {
      ...
      clusterData.nodeIds.forEach((nodeId) => {
        ...
        let forceVec = vecScalarMult(
          vecSub(clusterCenter, node),
...
self.onmessage = function (event) {
  ...
  switch (type) {
    ...
    case "removeEdge":
      edges = edges.filter(
        (e) =>
          !(e.sourceId === payload.sourceId && e.targetId === payload.targetId),
      );
      if (isRunning) lastKickTime = Date.now();
      break;
