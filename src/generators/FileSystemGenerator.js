...
export class FileSystemGenerator {
  ...
  _traverseFsItem(item, space, config, position, parentNode = null) {
    ...
    const nodeType =
      item.type === "directory"
        ? config.nodeTypeMapping.directory
        : config.nodeTypeMapping.file;
    ...
    if (parentNode && currentNode) {
      space.addEdge(parentNode, currentNode, {
        type: "straight",
        color: 0xaaaaaa,
        thickness: 1.5,
      });
    ...
    if (
      ...
      if (createType === "group" && currentNode) {
        ...
        item.children.forEach((childItem, index) => {
          ...
          const childNode = this._traverseFsItem(
            childItem,
            space,
            config,
            childPosition,
