...
export class CameraPlugin extends Plugin {
  ...
  init() {
    ...
    this.advancedControls = new AdvancedCameraControls(
      this.space,
      this.cameraControls,
    );
  ...
  _determineFocusNodeDistance(node) {
    ...
    const nodeSize =
      typeof node.getBoundingSphereRadius === "function"
        ? node.getBoundingSphereRadius() * 2
        : 100;
    ...
    return Math.max(
      minDistance,
      (projectedSize * paddingFactor) / (2 * Math.tan(fov / 2)),
    );
  }
