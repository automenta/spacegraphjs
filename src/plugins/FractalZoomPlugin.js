...
export class FractalZoomPlugin extends Plugin {
  ...
  _subscribeToEvents() {
    ...
    this.space.on(
      "fractal-zoom:levelChanged",
      this._onZoomLevelChanged.bind(this),
    );
  ...
  _onNodeAdded(nodeInstance) {
    ...
    try {
      ...
      console.error(
        `FractalZoomPlugin: Error creating default content adapter for node ${nodeInstance.id}:`,
        error,
      );
    }
  }
  ...
  _onNodeRemoved(nodeId, node) {
    ...
    try {
      ...
      console.error(
        `FractalZoomPlugin: Error during _onNodeRemoved for node ${nodeId}:`,
        error,
      );
    }
  }
  ...
  _createDefaultContentAdapter(nodeId, node) {
    try {
      ...
      if (typeof createContentAdapter !== "function") {
        console.error(
          "FractalZoomPlugin: createContentAdapter is not a function. Cannot create adapters.",
        );
        return;
      }
      ...
      if (text.length > 200) {
        ...
        if (adapter && typeof adapter.defineProgressiveText === "function") {
          ...
          console.warn(
            `FractalZoomPlugin: Failed to create or configure 'text' adapter for node ${nodeId}.`,
          );
        ...
        if (adapter) {
          ...
          console.warn(
            `FractalZoomPlugin: Failed to create 'data' adapter for node ${nodeId}.`,
          );
        }
      } else {
        ...
        if (adapter && typeof adapter.defineHTMLLevel === "function") {
          ...
          console.warn(
            `FractalZoomPlugin: Failed to create or configure 'html' adapter for node ${nodeId}.`,
          );
          adapter = null; // Ensure adapter is null if setup failed
        }
      }
      ...
      console.error(
        `FractalZoomPlugin: Error in _createDefaultContentAdapter for node ${nodeId}:`,
        error,
      );
    }
  }
  ...
  isTransitioning() {
    return this.fractalZoomManager
      ? this.fractalZoomManager.isTransitioningZoom()
      : false;
  }
  ...
  addContentAdapter(nodeId, adapter) {
    try {
      ...
      console.error(
        `FractalZoomPlugin: Error adding content adapter for node ${nodeId}:`,
        error,
      );
    }
  }
  ...
  removeContentAdapter(nodeId) {
    try {
      ...
      console.error(
        `FractalZoomPlugin: Error removing content adapter for node ${nodeId}:`,
        error,
      );
    }
  }
  ...
  updateConfig(newConfig) {
    ...
    if (this.fractalZoomManager) {
      ...
      this.fractalZoomManager.maxZoomIn = this.config.maxZoomIn;
      this.fractalZoomManager.maxZoomOut = this.config.maxZoomOut;
      this.fractalZoomManager.transitionDuration =
  ...
  getCurrentLODConfig() {
    return this.fractalZoomManager
      ? this.fractalZoomManager.getCurrentLODConfig()
      : null;
  }
    ...
    try {
      ...
      this.contentAdapters.forEach((adapter) => {
        try {
          ...
          console.error(
            "FractalZoomPlugin: Error disposing an content adapter:",
            adapterError,
          );
        }
      });
