...
export class DataPlugin extends Plugin {
  ...
  async importGraphFromJSON(jsonData, options = { clearExistingGraph: true }) {
    ...
    if (!nodePlugin || !edgePlugin || !layoutPlugin) {
      console.error(
        "DataPlugin: Required plugins (Node, Edge, Layout) not available.",
      );
      return false;
    }
    ...
    if (options.clearExistingGraph) {
      [...nodePlugin.getNodes().keys()].forEach((id) =>
        nodePlugin.removeNode(id),
      );
    }
    ...
    for (const edgeData of graphData.edges) {
      ...
      if (sourceNode && targetNode) {
        ...
        console.warn(
          `DataPlugin: Missing source/target node for edge:`,
          edgeData,
        );
      }
    }
