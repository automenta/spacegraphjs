...
export class MinimapPlugin extends Plugin {
  ...
  _setupFrustumHelper() {
    ...
    if (mainCamera) {
      ...
      console.warn(
        "MinimapPlugin: Main camera not found for FrustumHelper setup.",
      );
    }
  }
  ...
  _addNodeProxy(node) {
    ...
    const nodeColor =
      node.data && node.data.color ? node.data.color : NODE_PROXY_COLOR;
  ...
  render(renderer) {
    if (!this.minimapCamera || !renderer) return;
