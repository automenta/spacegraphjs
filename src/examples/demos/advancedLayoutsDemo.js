...
function createGraph(space) {
  const demo = {
    title: "Advanced Layout Systems",
    description:
      "Constraint-based, nested, adaptive, and connected layouts with morphing transitions",
    ...
    setupAdvancedLayoutManager() {
      ...
      space.on("layout:adapted", (event) => {
        console.log(
          `Layout adapted from ${event.from} to ${event.to} (${event.reason})`,
        );
        this.updateLayoutInfo();
      });
    },
    ...
    registerLayoutRegions() {
      ...
      const hierarchyNodes = this.nodes.filter(
        (n) => n.data.group === "hierarchy",
      );

      if (groupANodes.length > 0) {
        const boundsA = this.calculateNodeGroupBounds(groupANodes);
        this.layoutManager.registerLayoutRegion(
          "groupA",
          boundsA,
          "force",
          groupANodes,
    ...
    addLayoutConnections() {
      ...
      if (groupANodes.length > 0 && groupBNodes.length > 0) {
        for (let i = 0; i < 3; i++) {
          const sourceNode =
            groupANodes[Math.floor(Math.random() * groupANodes.length)];
          const targetNode =
            groupBNodes[Math.floor(Math.random() * groupBNodes.length)];
        ...
        case "angle":
          if (this.nodes.length >= 3) {
            const nodes = this.nodes
              ...
              .slice(0, 3);
            this.layoutManager.addAngleConstraint(
              nodes[0].id,
              nodes[1].id,
              nodes[2].id,
              {
    ...
    addContainerNode() {
      ...
      const nodesToAdd = availableNodes.slice(
        0,
        Math.min(6, availableNodes.length),
      );
    ...
    updateLayoutInfo(event) {
      ...
      document.getElementById("layoutMetrics").textContent =
