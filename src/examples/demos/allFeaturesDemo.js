...
function setupDemoMenu(space) {
  ...
  if (
    uiPlugin &&
    uiPlugin.uiManager &&
    uiPlugin.uiManager.hudManager &&
    uiPlugin.uiManager.hudManager.menuBar
  ) {
    ...
    if (demoMenu) {
      ...
      actionsSection.addMenuItem("addRandomNode", "Add Random Node", () => {
        ...
        if (type === "image")
          data.imageUrl = "https://placehold.co/100x50.png?text=Rnd";
        if (type === "video")
          data.videoUrl =
      ...
      console.warn(
        "Demo Menu could not be added. MenuBar not available as expected.",
      );
    }
  } else {
    console.warn(
      "AdvancedHudManager or MenuBar not found, skipping demo menu setup.",
    );
  }
}
...
function createGraph(space) {
  ...
  const n_features = space.createNode({
    id: "features",
    type: "html",
    position: { x: 350, y: 100, z: 20 },
    data: {
      content:
  ...
  const n_box = space.createNode({
    id: "box1",
    type: "shape",
    position: { x: 600, y: 150, z: 30 },
    data: {
  ...
  space.addEdge(n_features, n_box, {
    constraintType: "rigid",
    constraintParams: { distance: 180, stiffness: 0.08 },
  });

  const n_sphere = space.createNode({
    id: "sphere1",
    type: "shape",
    position: { x: 650, y: 0, z: -20 },
    data: {
  ...
  const n_tech = space.createNode({
    id: "tech",
    type: "html",
    position: { x: -350, y: 100, z: -10 },
    data: {
      content:
  ...
  const n_style = space.createNode({
    id: "style",
    type: "html",
    position: { x: 0, y: -250, z: 0 },
    data: {
      content:
        "<h2>Style 🎨</h2><p>✨ Dark/Transparent BG</p><p>🎨 Node Colors</p><p>🕸️ Dot Grid BG</p>",
      width: 220,
      height: 110,
      backgroundColor: nextColor(),
    },
  ...
  const n_interactive = space.createNode({
    id: "interactive",
    type: "html",
    position: { x: 350, y: -150, z: -30 },
    data: {
  ...
  space.addEdge(n_style, n_interactive, {
    constraintType: "elastic",
    constraintParams: { idealLength: 250 },
  });
