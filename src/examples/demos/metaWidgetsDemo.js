...
export function createGraph(space) {
  ...
  const controlCenter = WidgetComposer.createControlCenter(
    space,
    { x: -600, y: 150, z: 0 },
    [
      {
  ...
  const monitorDash = WidgetComposer.createMonitoringDashboard(
    space,
    { x: 600, y: 150, z: 0 },
    [
      {
  ...
  // Analytics Dashboard
  const analyticsDash = WidgetComposer.createAnalyticsDashboard(
    space,
    { x: -300, y: -200, z: 0 },
    {
      keyMetrics: [
  ...
  // Workflow Builder
  const workflowBuilder = WidgetComposer.createWorkflowBuilder(
    space,
    { x: 300, y: -200, z: 0 },
    [
  ...
  // Game HUD example
  const gameHUD = WidgetComposer.createGameHUD(
    space,
    { x: 0, y: -350, z: 0 },
    {
      health: 87,
  ...
  const flowConnection1 = WidgetComposer.connectWidgets(
    space,
    controlCenter,
    monitorDash,
    "data",
  );
  const flowConnection2 = WidgetComposer.connectWidgets(
    space,
    monitorDash,
    analyticsDash,
    "data",
  );
  const flowConnection3 = WidgetComposer.connectWidgets(
    space,
    analyticsDash,
    workflowBuilder,
    "control",
  ...
  space.on("meta-widget:control-changed", (event) => {
    ...
    console.log(
      `MetaWidget "${metaWidget.data.title}" control "${controlId}" changed to:`,
      value,
    );
  ...
  space.on("meta-widget:control-clicked", (event) => {
    ...
    console.log(
      `MetaWidget "${metaWidget.data.title}" button "${controlId}" clicked`,
    );
  ...
  infoNodes.forEach((info) => {
    space.addNode(
      new S.TextMeshNode(info.id, info.position, {
        text: info.text,
        fontSize: 11,
        height: 3,
        color: info.color,
