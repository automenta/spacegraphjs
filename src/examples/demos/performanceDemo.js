...
export const demoMetadata = {
  ...
  description:
    "Large-scale graph with instancing, culling, LOD, and web workers",
};
...
function createPerformanceDemo(space) {
  ...
  if (!performancePlugin) {
    console.warn(
      "PerformancePlugin not found - performance features will not be available",
    );
  }
  ...
  return {
    ...
    description:
      "Large-scale graph demonstrating instancing, culling, LOD, and web workers",
    instructions: [
...
function createLargeNodeClusters(space) {
  ...
  console.log(
    `Created ${clusterCount} clusters with ${nodesPerCluster} nodes each`,
  );
}
...
function createControlPanel(space) {
  ...
  setTimeout(() => {
    ...
    // Feature toggles
    panel
      .querySelector("#instancing-toggle")
    ...
    panel
      .querySelector("#stress-test-button")
