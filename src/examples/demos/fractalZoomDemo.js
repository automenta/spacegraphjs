...
export const demoMetadata = {
  ...
  title: "🌀 Fractal Zoom",
  description:
    "Infinite zoom with level-of-detail management and content adaptation",
};
...
function createFractalZoomDemo(space) {
  ...
  if (!fractalZoomPlugin) {
    console.warn(
      "FractalZoomPlugin not found - fractal zoom features will not be available",
    );
  }
  ...
  const textData = [
    {
      ...
      summary: "AI Overview",
      detail:
        "Machine learning, neural networks, and intelligent systems for automation and decision making.",
      full: `
...
function setupContentAdapters(space) {
  ...
  space.getNodes().forEach((node) => {
    ...
    if (node.fractalData.type === "data") {
      ...
      adapter = createContentAdapter(node.id, "data", {
        data: node.fractalData.data,
      });
      ...
      adapter.defineHTMLLevel(
        -10,
        10,
        node.fractalData.content || node.fractalData.summary,
      );
    }
...
function addZoomInstructions(space) {
  ...
  if (fractalZoomPlugin) {
    ...
    adapter.defineHTMLLevel(
      -10,
      -2,
      '<div class="zoom-help">🌀 Zoom Controls</div>',
    );
