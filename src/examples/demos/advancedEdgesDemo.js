...
function createAdvancedDynamicEdges(
  space,
  ...
  const animateAdvEdges = () => {
    ...
    edges.springs.forEach((edge, index) =>
      edge.setRestLength?.(300 + Math.sin(animationTime * 0.5 + index) * 50),
    );
    edges.beziers.forEach((edge, index) =>
      edge.setCurveTension?.(
        Math.max(0.1, 0.4 + Math.sin(animationTime * 0.3 + index * 0.5) * 0.2),
      ),
    );
    if (document.getElementById(demoMetadata.id))
      requestAnimationFrame(animateAdvEdges); // Continue if demo is active
  };
  animateAdvEdges();
  return centralNode;
}
...
function createSpecializedStaticEdges(
  space,
  basePosition = { x: 800, y: -600, z: 0 },
) {
  const commonProps = {
    mass: 1.0,
    type: "shape",
  ...
  const n1 = space.createNode({
    ...
    data: { ...commonProps.data, label: "SSE1", color: 0xff8888 },
    ...commonProps,
  });
  const n2 = space.createNode({
    ...
    data: { ...commonProps.data, label: "SSE2", color: 0x88ff88 },
    ...commonProps,
  });
  const n3 = space.createNode({
    ...
    data: { ...commonProps.data, label: "SSE3", color: 0x8888ff },
    ...commonProps,
  });
