...
export class EnhancedDemoRunner {
  ...
  _determineComplexity(page) {
    ...
    if (
      ...
      return "Advanced";
    if (id.includes("meta") || id.includes("layout") || id.includes("camera"))
      return "Intermediate";
            ...
            <div class="sidebar-header">
                ...
                <div class="filter-container">
                    <select id="category-filter">
                        ...
                        ${Array.from(this.demoCategories.keys())
                          .map(
  ...
  _bindEvents() {
    ...
    searchInput?.addEventListener("input", (e) =>
      this._filterDemos(e.target.value),
    );
    ...
    categoryFilter?.addEventListener("change", (e) =>
      this._filterByCategory(e.target.value),
    );
    complexityFilter?.addEventListener("change", (e) =>
      this._filterByComplexity(e.target.value),
    );

    // Toolbar controls
    document
      .getElementById("demo-home")
      ?.addEventListener("click", () => this._showHome());
    document
      .getElementById("demo-back")
      ?.addEventListener("click", () => this._navigateDemo(-1));
    document
      .getElementById("demo-forward")
      ?.addEventListener("click", () => this._navigateDemo(1));
    document
      .getElementById("demo-reload")
      ?.addEventListener("click", () => this._reloadCurrentDemo());
    document
      .getElementById("demo-fullscreen")
      ?.addEventListener("click", () => this._toggleFullscreen());
    document
      .getElementById("show-code")
      ?.addEventListener("click", () => this._toggleCodeViewer());
    document
      .getElementById("show-performance")
      ?.addEventListener("click", () => this._togglePerformanceMonitor());
    document
  ...
  _filterDemos(searchTerm) {
    ...
    demoItems.forEach((item) => {
      ...
      const description = item
        .querySelector(".demo-description")
  ...
  _updateCategoryVisibility() {
    ...
    categorySections.forEach((section) => {
      const visibleDemos = section.querySelectorAll(
        '.demo-item[style*="block"], .demo-item:not([style])',
      );
  ...
  _navigateDemo(direction) {
    ...
    const currentIndex = this.enhancedPages.findIndex(
      (p) => p.id === this.currentDemo.id,
    );
  ...
  _startPerformanceMonitoring() {
    if (this.space.performance) {
      this.space.on(
        "performance:update",
        this._updatePerformanceDisplay.bind(this),
      );
    }
  }
