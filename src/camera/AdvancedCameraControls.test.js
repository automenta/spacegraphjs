...
describe("AdvancedCameraControls", () => {
  ...
  beforeEach(() => {
    ...
    mockSpace = {
      ...
      plugins: {
        // Mocking plugins structure if space.plugins.getPlugin is used
  ...
  it("should manage cinematic keyframes (internally)", () => {
    const nodeMapWithOneNode = new Map([
      ["node1", { id: "node1", position: new mockThree.Vector3(0, 0, 0) }],
    ]);
  ...
  it("should update camera position during auto-zoom", () => {
    ...
    mockNodePlugin.getNodes = vi.fn(
      () =>
        new Map([
          [
            "node1",
            {
              position: new mockThree.Vector3(0, 0, 0),
          ...
          [
            "node2",
            {
              position: new mockThree.Vector3(100, 0, 0),
              getBoundingSphereRadius: () => 10,
            },
          ],
        ]),
