...
export class AdvancedCameraControls {
  ...
  toggleAutoZoom(enabled = null) {
    ...
    this.space.emit("camera:autoZoomToggled", {
  ...
  _performAutoZoom() {
    ...
    const optimalDistance = this._calculateOptimalZoomDistance(
      boundingBox,
      nodes.length,
    );
  ...
  _calculateOptimalZoomDistance(boundingBox, nodeCount) {
    ...
    const distance =
      (maxDimension * this.settings.autoZoom.targetPadding) /
      (2 * Math.tan(fov / 2));
  ...
  toggleAutoRotation(enabled = null) {
    this.settings.rotation.autoRotate =
      enabled !== null ? enabled : !this.settings.rotation.autoRotate;

    if (!this.settings.rotation.autoRotate) {
      this.autoRotateAngle = 0;
    }

    this.space.emit("camera:autoRotationToggled", {
      enabled: this.settings.rotation.autoRotate,
    });
  ...
  _updateRotation() {
    if (
      this.settings.rotation.autoRotate &&
      this.cameraControls.cameraMode === CAMERA_MODES.ORBIT
    ) {
    ...
    this.currentRotation.lerp(
      this.targetRotation,
      this.settings.rotation.smoothDamping,
    );
  }

  // Peek around corners functionality
  togglePeekMode(enabled = null) {
    this.settings.peekMode.enabled =
      enabled !== null ? enabled : !this.settings.peekMode.enabled;

    if (!this.settings.peekMode.enabled && this.isPeeking) {
      this._exitPeekMode();
    }

    this.space.emit("camera:peekModeToggled", {
      enabled: this.settings.peekMode.enabled,
    });
  ...
  _updatePeekMode() {
    ...
    const mouseMovement = this.mousePosition
      .clone()
    ...
    const nearEdge =
      Math.abs(this.mousePosition.x) > 0.7 ||
      Math.abs(this.mousePosition.y) > 0.7;
  ...
  _enterPeekMode() {
    ...
    if (!this.camera) {
      console.warn(
        "AdvancedCameraControls: Camera not available for peek mode.",
      );
      return;
    }
  ...
  _updatePeekDirection() {
    ...
    if (!this.camera) {
      console.warn(
        "AdvancedCameraControls: Camera not available for peek direction update.",
      );
      return;
    }
  ...
  toggleCinematicMode(enabled = null) {
    ...
    this.space.emit("camera:cinematicModeToggled", {
  ...
  _updateCinematicMode() {
    ...
    this.cinematicProgress +=
      this.settings.cinematic.cinematicSpeed * this.cinematicDirection * 0.01;
  ...
  smartFocusOnNode(node, options = {}) {
    ...
    if (!node.position) {
      console.warn(
        "AdvancedCameraControls.smartFocusOnNode: node.position is undefined for the primary node.",
      );
      return;
    }

    let focusArea = new THREE.Box3();
    focusArea.setFromCenterAndSize(
      node.position,
    ...
    if (considerNeighbors) {
      ...
      edges.forEach((edge) => {
        if (
          edge.source === node &&
          edge.target.position.distanceTo(node.position) < contextRadius
        ) {
          connectedNodes.add(edge.target);
        }
        if (
          edge.target === node &&
          edge.source.position.distanceTo(node.position) < contextRadius
        ) {
      ...
      connectedNodes.forEach((connectedNode) => {
        if (!connectedNode.position) {
          console.warn(
            "AdvancedCameraControls.smartFocusOnNode: connectedNode.position is undefined for a connected node.",
          );
  ...
  dispose() {
    ...
    this.space.container.removeEventListener(
      "mousemove",
      this._boundHandleMouseMove,
    );
    this.space.container.removeEventListener(
