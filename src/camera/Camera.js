...
export class Camera {
  ...
  constructor(space) {
    if (!space?._cam || !space.container)
      throw new Error("Camera requires SpaceGraph instance.");
    this.space = space;
  ...
  _initializePointerLockControls() {
    ...
    this.pointerLockControls.addEventListener("lock", () => {
      ...
      Object.keys(this.moveState).forEach(
        (key) => (this.moveState[key] = false),
      );
    ...
    this.pointerLockControls.addEventListener("unlock", () => {
      ...
      this.domElement.style.cursor =
        this.cameraMode === CAMERA_MODES.FREE ? "crosshair" : "grab";
  ...
  startPan(startX, startY) {
    if (
      (this.cameraMode !== CAMERA_MODES.ORBIT &&
        this.cameraMode !== CAMERA_MODES.TOP_DOWN &&
        this.cameraMode !== CAMERA_MODES.DRAG_ORBIT) ||
      this.isPanning ||
    ...
    if (this.isFollowing && this.followOptions.autoEndOnManualControl)
      this.stopFollowing();
  ...
  zoom(deltaY) {
    ...
    if (this.cameraMode === CAMERA_MODES.TOP_DOWN) {
      ...
      const lookAtToCam = new THREE.Vector3().subVectors(
        this.targetPosition,
        this.targetLookAt,
      );
      const currentDist = lookAtToCam.length();
      const newDist = Utils.clamp(
      ...
      this.targetPosition
        ...
        .addScaledVector(lookAtToCam.normalize(), newDist);
    }
  }
  ...
  moveTo(x, y, z, duration = 0.7, lookAtTarget = null, newCameraMode = null) {
    ...
    if (newCameraMode && newCameraMode !== this.cameraMode)
      this.setCameraMode(newCameraMode, true);
  }
  ...
  _updateCameraLogic = () => {
    ...
    if (
      (this.cameraMode === CAMERA_MODES.FREE ||
    ...
    if (needsLerp) {
      ...
      if (this.currentPosition.distanceTo(this.targetPosition) <= epsilon)
        this.currentPosition.copy(this.targetPosition);
      if (this.currentLookAt.distanceTo(this.targetLookAt) <= epsilon)
        this.currentLookAt.copy(this.targetLookAt);
  ...
  startFollowing(target, options = {}) {
    ...
    gsap.killTweensOf(this.targetPosition);
    gsap.killTweensOf(this.targetLookAt);
    this.space.emit("camera:followStarted", {
      target: this.followTargetObject,
      options: this.followOptions,
    });
  }
  ...
  _saveNamedViewsToStorage() {
    try {
      ...
      localStorage.setItem(
        "spacegraph_namedViews",
        JSON.stringify(viewsToStore),
      );
    } catch (e) {
  ...
  saveNamedView(name) {
    ...
    this.space.emit("camera:namedViewSaved", {
      name,
      view: this.namedViews.get(name),
    });
    return true;
  }
  ...
  setCameraMode(mode, calledInternally = false) {
    ...
    if (
      oldMode === CAMERA_MODES.FREE ||
      oldMode === CAMERA_MODES.FIRST_PERSON
    ) {
    ...
    switch (this.cameraMode) {
      ...
      case CAMERA_MODES.FREE:
        ...
        const lookDirectionFree = new THREE.Vector3(0, 0, -1).applyQuaternion(
          this._cam.quaternion,
        );
      ...
      case CAMERA_MODES.FIRST_PERSON:
        this.domElement.style.cursor = "crosshair";
        this.targetPosition.copy(this.currentPosition);
        const lookDirectionFPS = new THREE.Vector3(0, 0, -1).applyQuaternion(
          this._cam.quaternion,
        );
    ...
    this.space.emit("camera:modeChanged", {
      newMode: this.cameraMode,
      oldMode,
    });
  ...
  orbitDrag(deltaX, deltaY) {
    if (this.cameraMode !== CAMERA_MODES.DRAG_ORBIT || !this.isOrbitDragging)
      return;
    ...
    const cameraX = new THREE.Vector3().setFromMatrixColumn(
      this._cam.matrixWorld,
      0,
    );
    ...
    if (angleToUp > minPolar && angleToUp < maxPolar) {
      this.targetPosition
        .copy(this.currentLookAt)
        .add(lookAtToCam.setLength(radius));
    } else {
  ...
  endOrbitDrag() {
    ...
    this._isManuallyControlled = false;
    this.domElement.style.cursor =
      this.cameraMode === CAMERA_MODES.DRAG_ORBIT ? "grab" : "default";

    // Re-enable pointer lock if it was active before orbit drag
    if (
      this.prevPointerLockControlsEnabled &&
