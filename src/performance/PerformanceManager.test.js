...
vi.mock("three", async (importOriginal) => {
  ...
  const MockBox3 = vi.fn(function (
    ...
    this.getCenter = vi.fn((target) =>
      target
        ? target.copy(this.min).add(this.max).multiplyScalar(0.5)
        : new MockVector3().copy(this.min).add(this.max).multiplyScalar(0.5),
    );
    this.getSize = vi.fn((target) =>
      target
        ? target.subVectors(this.max, this.min)
        : new MockVector3().subVectors(this.max, this.min),
    );
    this.isEmpty = vi.fn(() => false);
...
describe("PerformanceManager", () => {
  ...
  beforeEach(() => {
    ...
    mockNodes = [
      {
        id: "node1",
        position: new THREE_MOCKED.Vector3(0, 0, 0),
        object3d: {
          visible: true,
          children: [],
          matrixWorld: new THREE_MOCKED.Matrix4(),
          geometry: {
            boundingSphere: {
              center: new THREE_MOCKED.Vector3(),
              radius: 1,
              applyMatrix4: vi.fn(),
            },
          },
    ...
    mockEdges = [
      {
        id: "edge1",
        position: new THREE_MOCKED.Vector3(0, 0, 0),
        object3d: { visible: true, children: [] },
      },
    ];
