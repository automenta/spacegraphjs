...
export class PerformanceManager {
  ...
  _updateStats() {
    ...
    this.stats.culledObjects =
      this.stats.totalObjects - this.stats.visibleObjects;
...
class InstanceManager {
  ...
  unregisterObject(object) {
    ...
    if (group) {
      ...
      if (
        group.objects.length < this.perfManager.config.instanceThreshold &&
        group.instancedMesh
      ) {
  ...
  _createInstancedMesh(geometryKey) {
    ...
    const maxInstances = Math.min(
      ...
      this.perfManager.config.maxInstances,
    );
...
class CullingManager {
  ...
  init(renderingPlugin) {
    ...
    const cameraPlugin =
      this.perfManager.space.plugins.getPlugin("CameraPlugin");
    if (cameraPlugin) {
  ...
  updateCulling() {
    ...
    if (
      currentPos.distanceTo(this.lastCameraPosition) < this.cameraMovedThreshold
    ) {
      return;
    }
...
class MemoryManager {
  ...
  _performCleanup() {
    ...
    if (this.cachedObjects.size > this.perfManager.config.maxCachedObjects) {
      const excess =
        this.cachedObjects.size - this.perfManager.config.maxCachedObjects;
      const keysToDelete = Array.from(this.cachedObjects.keys()).slice(
        0,
        excess,
      );
