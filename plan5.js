...
class PlanningProblem {
    ...
    applyPlan(state, plan) {
        plan.forEach(node => {
            state = this.applyAction(state, node.action, node.params);
        });
        return state;
    }
}
...
class HeuristicSearch extends SearchStrategy {
    async search(problem) {
        const openSet = new Set([[new Set(problem.init), this.heuristic(problem.init, problem.goal)]]);
        ...
        while (openSet.size > 0) {
            ...
            if (problem.isGoalSatisfied(current)) {
                ...
                while (cameFrom.has(state)) {
                    const { action, params } = cameFrom.get(state);
                    path.push(new PlanNode(action, params));
                    state = problem.applyAction(state, action, params);
                }
                return path.reverse();
            }
            ...
            for (const action of applicableActions) {
                const successor = problem.applyAction(current, action, action.params);
                const tentativeGScore = gScore.get(current) + action.duration;

                if (!gScore.has(successor) || tentativeGScore < gScore.get(successor)) {
                    cameFrom.set(successor, { action, params: action.params });
                    gScore.set(successor, tentativeGScore);
                    fScore.set(successor, tentativeGScore + this.heuristic(successor, problem.goal));
                    openSet.add([successor, fScore.get(successor)]);
                }
            }
        }
    ...
    async search(problem) {
        ...
        while (true) {
            ...
            if (bestNeighbor === null) {
                return problem.applyPlan(problem.init, current);
            }
...
function heuristic(state, goal) {
    ...
    return Array.from(goal).filter(predicate => !state.has(predicate.toString())).length;
}

// Usage example
const problem = new PlanningProblem(
    new Set().add('location'),
    new Set().add('agent1').add('location1').add('location2').add('location3'),
    new Set().add(new Predicate('at', 'agent1', 'location1')),
    new Set().add(new Predicate('at', 'agent1', 'location3')),
...
plan.forEach(node => console.log(`  ${node}`));

// Additional usage examples
const bfsPlanner = new Planner(problem, new BreadthFirstSearch());
const bfsPlan = await bfsPlanner.plan();
console.log('BFS Plan:');
bfsPlan.forEach(node => console.log(`  ${node}`));

const dfsPlanner = new Planner(problem, new DepthFirstSearch());
const dfsPlan = await dfsPlanner.plan();
console.log('DFS Plan:');
dfsPlan.forEach(node => console.log(`  ${node}`));

const localPlanner = new Planner(problem, new LocalSearch(heuristic));
const localPlan = await localPlanner.plan();
console.log('Local Search Plan:');
localPlan.forEach(node => console.log(`  ${node}`));

const gaPlanner = new Planner(problem, new GeneticAlgorithm(heuristic));
const gaPlan = await gaPlanner.plan();
console.log('Genetic Algorithm Plan:');
gaPlan.forEach(node => console.log(`  ${node}`));
